#!/bin/bash

#############################################################################
# PostgreSQL User Permissions Tester (Selectel Edition)
#
# Описание:
# Тестирует права пользователя PostgreSQL:
# - READ/WRITE доступ к своей схеме
# - Отсутствие доступа к другим схемам
# - Корректность подключения
#
# Использование:
# ./test-schema-permissions.sh <service_name> <environment>
#
# Примеры:
# ./test-schema-permissions.sh auth_service staging
# ./test-schema-permissions.sh profile_service prod
#############################################################################

set -e

# Цвета
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
CYAN='\033[0;36m'
MAGENTA='\033[0;35m'
NC='\033[0m'

log_info() { echo -e "${BLUE}ℹ${NC} $1"; }
log_success() { echo -e "${GREEN}✅${NC} $1"; }
log_warning() { echo -e "${YELLOW}⚠${NC} $1"; }
log_error() { echo -e "${RED}❌${NC} $1"; }
log_test() { echo -e "${MAGENTA}🧪${NC} $1"; }

#############################################################################
# Проверка параметров
#############################################################################

SERVICE_NAME=$1
ENVIRONMENT=$2

if [ -z "$SERVICE_NAME" ] || [ -z "$ENVIRONMENT" ]; then
    log_error "Недостаточно параметров!"
    echo ""
    echo "Использование: ./test-schema-permissions.sh <service_name> <environment>"
    echo ""
    echo "Примеры:"
    echo "  ./test-schema-permissions.sh auth_service staging"
    echo "  ./test-schema-permissions.sh profile_service prod"
    echo ""
    exit 1
fi

if [ "$ENVIRONMENT" != "staging" ] && [ "$ENVIRONMENT" != "prod" ]; then
    log_error "Окружение должно быть 'staging' или 'prod'"
    exit 1
fi

#############################################################################
# Формирование имён
#############################################################################

SCHEMA_NAME="${SERVICE_NAME}_${ENVIRONMENT}"
USER_APP="${SERVICE_NAME}_${ENVIRONMENT}_app"
SERVICE_NAME_DASHED=$(echo "$SERVICE_NAME" | tr '_' '-')
SECRET_NAME="postgres-${SERVICE_NAME_DASHED}-${ENVIRONMENT}-app"
K8S_NAMESPACE="postgres"

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
log_info "Тестирование прав пользователя: ${CYAN}${USER_APP}${NC}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "📋 Параметры теста:"
echo ""
echo -e "  Схема:        ${CYAN}${SCHEMA_NAME}${NC}"
echo -e "  Пользователь: ${CYAN}${USER_APP}${NC}"
echo -e "  Secret:       ${CYAN}${SECRET_NAME}${NC}"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

#############################################################################
# Получение данных из Kubernetes Secret
#############################################################################

log_info "Получение данных из Kubernetes Secret..."

if ! kubectl get secret "$SECRET_NAME" -n "$K8S_NAMESPACE" &>/dev/null; then
    log_error "Secret '$SECRET_NAME' не найден в namespace '$K8S_NAMESPACE'"
    exit 1
fi

PGHOST=$(kubectl get secret "$SECRET_NAME" -n "$K8S_NAMESPACE" -o jsonpath='{.data.host}' | base64 -d)
PGPORT=$(kubectl get secret "$SECRET_NAME" -n "$K8S_NAMESPACE" -o jsonpath='{.data.port}' | base64 -d)
PGDATABASE=$(kubectl get secret "$SECRET_NAME" -n "$K8S_NAMESPACE" -o jsonpath='{.data.dbname}' | base64 -d)
PGUSER=$(kubectl get secret "$SECRET_NAME" -n "$K8S_NAMESPACE" -o jsonpath='{.data.user}' | base64 -d)
PGPASSWORD=$(kubectl get secret "$SECRET_NAME" -n "$K8S_NAMESPACE" -o jsonpath='{.data.password}' | base64 -d)
PGSCHEMA=$(kubectl get secret "$SECRET_NAME" -n "$K8S_NAMESPACE" -o jsonpath='{.data.schema}' | base64 -d)

if [ -z "$PGHOST" ] || [ -z "$PGPASSWORD" ]; then
    log_error "Не удалось получить данные из Secret"
    exit 1
fi

log_success "Данные получены из Secret"
echo "  Host: $PGHOST"
echo "  Database: $PGDATABASE"
echo "  User: $PGUSER"
echo "  Schema: $PGSCHEMA"
echo ""

#############################################################################
# Счётчики тестов
#############################################################################

TESTS_PASSED=0
TESTS_FAILED=0
TESTS_TOTAL=0

#############################################################################
# Функция для выполнения теста
#############################################################################

run_test() {
    local TEST_NAME="$1"
    local SQL_COMMAND="$2"
    local EXPECTED_SUCCESS="$3"  # true или false

    TESTS_TOTAL=$((TESTS_TOTAL + 1))
    log_test "Тест $TESTS_TOTAL: $TEST_NAME"

    RESULT=$(kubectl exec -i deployment/bastion -n bastion -- bash -c \
        "PGPASSWORD='$PGPASSWORD' psql -h '$PGHOST' -p '$PGPORT' -U '$PGUSER' -d '$PGDATABASE' -tAc \"$SQL_COMMAND\" 2>&1" || echo "ERROR")

    if [ "$EXPECTED_SUCCESS" = "true" ]; then
        if [[ "$RESULT" != *"ERROR"* ]] && [[ "$RESULT" != *"permission denied"* ]] && [[ "$RESULT" != *"does not exist"* ]]; then
            log_success "Пройден"
            TESTS_PASSED=$((TESTS_PASSED + 1))
            return 0
        else
            log_error "Провален: $RESULT"
            TESTS_FAILED=$((TESTS_FAILED + 1))
            return 1
        fi
    else
        # Ожидаем провал
        if [[ "$RESULT" == *"permission denied"* ]] || [[ "$RESULT" == *"must be owner"* ]] || [[ "$RESULT" == *"does not exist"* ]]; then
            log_success "Пройден (доступ корректно запрещён)"
            TESTS_PASSED=$((TESTS_PASSED + 1))
            return 0
        else
            log_error "Провален: доступ не был запрещён"
            TESTS_FAILED=$((TESTS_FAILED + 1))
            return 1
        fi
    fi
}

#############################################################################
# Тест 1: Подключение к базе данных
#############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔍 Категория 1: Базовое подключение"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

run_test "Подключение к базе данных" "SELECT current_user;" true
run_test "Проверка текущей схемы" "SET search_path TO '$SCHEMA_NAME'; SELECT current_schema();" true

echo ""

#############################################################################
# Тест 2: READ/WRITE права в своей схеме
#############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔍 Категория 2: READ/WRITE права в схеме $SCHEMA_NAME"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

# Создание таблицы
run_test "CREATE TABLE в своей схеме" \
    "SET search_path TO '$SCHEMA_NAME'; CREATE TABLE test_permissions_table (id SERIAL PRIMARY KEY, name VARCHAR(50), created_at TIMESTAMP DEFAULT NOW());" true

# Вставка данных
run_test "INSERT данных в таблицу" \
    "SET search_path TO '$SCHEMA_NAME'; INSERT INTO test_permissions_table (name) VALUES ('test_user_1'), ('test_user_2');" true

# Чтение данных
run_test "SELECT данных из таблицы" \
    "SET search_path TO '$SCHEMA_NAME'; SELECT COUNT(*) FROM test_permissions_table;" true

# Обновление данных
run_test "UPDATE данных в таблице" \
    "SET search_path TO '$SCHEMA_NAME'; UPDATE test_permissions_table SET name = 'updated_user' WHERE name = 'test_user_1';" true

# Удаление данных
run_test "DELETE данных из таблицы" \
    "SET search_path TO '$SCHEMA_NAME'; DELETE FROM test_permissions_table WHERE name = 'test_user_2';" true

# Создание индекса
run_test "CREATE INDEX на таблице" \
    "SET search_path TO '$SCHEMA_NAME'; CREATE INDEX idx_test_name ON test_permissions_table(name);" true

# Удаление таблицы
run_test "DROP TABLE в своей схеме" \
    "SET search_path TO '$SCHEMA_NAME'; DROP TABLE test_permissions_table;" true

echo ""

#############################################################################
# Тест 3: Работа с последовательностями (sequences)
#############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔍 Категория 3: Работа с SEQUENCES"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

run_test "CREATE SEQUENCE в своей схеме" \
    "SET search_path TO '$SCHEMA_NAME'; CREATE SEQUENCE test_seq START 1;" true

run_test "Использование SEQUENCE" \
    "SET search_path TO '$SCHEMA_NAME'; SELECT nextval('test_seq');" true

run_test "DROP SEQUENCE в своей схеме" \
    "SET search_path TO '$SCHEMA_NAME'; DROP SEQUENCE test_seq;" true

echo ""

#############################################################################
# Тест 4: Попытка доступа к другим схемам
#############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔍 Категория 4: Изоляция от других схем"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

# Получаем список других схем (не своя, не системные)
OTHER_SCHEMAS=$(kubectl exec -i deployment/bastion -n bastion -- bash -c \
    "PGPASSWORD=$PGPASSWORD psql -h $PGHOST -p $PGPORT -U $PGUSER -d $PGDATABASE -tAc \
    \"SELECT nspname FROM pg_namespace 
    WHERE nspname NOT LIKE 'pg_%' 
    AND nspname NOT IN ('$SCHEMA_NAME', 'information_schema', 'public') 
    LIMIT 3;\" 2>/dev/null" | tr -d ' ')

if [ -n "$OTHER_SCHEMAS" ]; then
    log_info "Найдены другие схемы для тестирования изоляции"
    echo ""

    while IFS= read -r OTHER_SCHEMA; do
        if [ -n "$OTHER_SCHEMA" ]; then
            # Тест 13: Попытка создать таблицу в чужой схеме
            run_test "Попытка CREATE TABLE в схеме $OTHER_SCHEMA (должна провалиться)" \
                "CREATE TABLE \"$OTHER_SCHEMA\".unauthorized_table (id INT);" false

            # Тест 14: Попытка прочитать данные из чужой схемы
            # Сначала получаем имя реальной таблицы из pg_tables
            TABLE_NAME=$(kubectl exec -i deployment/bastion -n bastion -- bash -c \
                "PGPASSWORD='$PGPASSWORD' psql -h '$PGHOST' -p '$PGPORT' -U '$PGUSER' -d '$PGDATABASE' -tAc \
                \"SELECT tablename FROM pg_tables WHERE schemaname = '$OTHER_SCHEMA' LIMIT 1;\" 2>/dev/null" | tr -d ' ')

            if [ -n "$TABLE_NAME" ]; then
                run_test "Попытка SELECT из таблицы $OTHER_SCHEMA.$TABLE_NAME (должна провалиться)" \
                    "SELECT * FROM \"$OTHER_SCHEMA\".\"$TABLE_NAME\" LIMIT 1;" false
            else
                log_warning "В схеме $OTHER_SCHEMA нет таблиц, тест SELECT пропущен"
            fi
        fi
    done <<< "$OTHER_SCHEMAS"
else
    log_warning "Других схем не найдено, тесты изоляции пропущены"
fi

echo ""
##
# Тест 5: Попытка создания схем и пользователей (должна провалиться)
#############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔍 Категория 5: Ограничения суперпользователя"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

run_test "Попытка CREATE SCHEMA (должна провалиться)" \
    "CREATE SCHEMA unauthorized_schema;" false

run_test "Попытка CREATE USER (должна провалиться)" \
    "CREATE USER unauthorized_user WITH PASSWORD 'test123';" false

# ИСПРАВЛЕНО: убран IF EXISTS, чтобы получить ошибку permission denied
run_test "Попытка DROP схемы public (должна провалиться)" \
    "DROP SCHEMA public CASCADE;" false

echo ""

#############################################################################
# Тест 6: Проверка доступа к системным таблицам
#############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔍 Категория 6: Доступ только для чтения к метаданным"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

run_test "Чтение своей схемы из information_schema" \
    "SELECT schema_name FROM information_schema.schemata WHERE schema_name = '$SCHEMA_NAME';" true

run_test "Просмотр своих таблиц" \
    "SELECT table_name FROM information_schema.tables WHERE table_schema = '$SCHEMA_NAME';" true

echo ""

#############################################################################
# Итоговая статистика
#############################################################################

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📊 ИТОГОВАЯ СТАТИСТИКА"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo -e "  Всего тестов: ${CYAN}${TESTS_TOTAL}${NC}"
echo -e "  Пройдено:     ${GREEN}${TESTS_PASSED}${NC}"
echo -e "  Провалено:    ${RED}${TESTS_FAILED}${NC}"
echo ""

if [ $TESTS_FAILED -eq 0 ]; then
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_success "ВСЕ ТЕСТЫ ПРОЙДЕНЫ! ✨"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    echo "✅ Подтверждено:"
    echo "  • Пользователь имеет полный доступ к своей схеме"
    echo "  • Пользователь НЕ имеет доступа к другим схемам"
    echo "  • Пользователь НЕ может создавать схемы/пользователей"
    echo "  • Права настроены корректно согласно принципу least privilege"
    echo ""
    exit 0
else
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    log_error "НЕКОТОРЫЕ ТЕСТЫ ПРОВАЛЕНЫ!"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    echo ""
    log_warning "Проверьте права пользователя и конфигурацию схемы"
    echo ""
    exit 1
fi
